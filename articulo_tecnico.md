...
Del ITER nacional se filtraron los registros municipales (`LOC == "0000"`) de la entidad 17, excluido el total estatal: exactamente **36 municipios**, incluidos los tres de reciente creación, verificado programáticamente. Los tipos de dato y valores especiales (`*`, N/D) se revisaron conforme al diccionario oficial, sin valores faltantes en las columnas seleccionadas.
...
El flujo fue estandarización con `StandardScaler` —proporciones, años y personas por vivienda tienen escalas distintas— y **KMeans** con `n_init = 10` y semilla fija (`random_state = 42`) para reproducibilidad.

El número de grupos se evaluó para k entre 2 y 10 con el codo (inercia) y la silueta. La silueta alcanza su máximo en k = 5 (0.303), pero esa solución produce grupos de solo 1 y 2 municipios —uno quedaría integrado únicamente por Hueyapan, aislado por su drenaje extremadamente bajo—, lo que no es interpretable ni útil. Se eligió **k = 3** (silueta = 0.273), donde el codo se flexiona, porque genera grupos interpretables y útiles para el contraste externo; la preferencia de la métrica por k = 5 queda documentada como parte de la decisión.
...
Los tres perfiles, renombrados por su nivel de conectividad, integran 9 (*alto*), 7 (*intermedio*) y 20 (*bajo*) municipios:
